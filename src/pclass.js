class MyPokedex {
    ...
    async FillDataRows() {
        ...
        this.names.forEach( (v, i) => {
          ...
          new Promise( (resolve, reject) => {
            this._P.getPokemonByName(v)
            .then(function(response) {
              //console.log(response.name, response.types);                           
              self.dataRows.push( self.MapResponse('getPokemonByName', response)
    ...
    MapResponse(functionCalledName, response) { // Get something from response
      let mappedResponse = { };      
      if('getPokemonByName' === functionCalledName) mappedResponse = 
    ...
    SortDataRows(nameColumn = undefined) {
      ...
      this.dataRows.sort( (a,b) => {
        let compared = 0;
        if(a[this.sortColumn] > b[this.sortColumn]) compared = 1 * this.sortDirection
        else if(a[this.sortColumn] < b[this.sortColumn]) compared = -1 * this.sortDirection
        return compared;
      });
    }
}
